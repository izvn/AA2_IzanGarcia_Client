...
// OJO PROFE: Uso bloqueante intencionadamente aquí. 
// Llamo a esta función durante la transición del main. No importa que el juego se pare medio segundo 
// mientras pillo la info, porque no hay nada animándose de fondo.
    ...
    if (resolvedIPs.has_value() && !resolvedIPs->empty()) {
        if (socket.connect((*resolvedIPs)[0], Config::BOOTSTRAP_PORT) == sf::Socket::Status::Done) {
            ...
            if (socket.receive(r) == sf::Socket::Status::Done) {
                std::uint32_t count;
                // Leo el número total de resultados y hago un for para parsear
                if (r >> count) {
                    rankingData.clear();
...
void RankingScreen::draw(sf::RenderWindow& window) {
    ...
    // Pintar los encabezados de las columnas (con coordenadas fijas en vez de intentar un sistema HTML dinámico, más seguro)
    ...
    for (size_t i = 0; i < rankingData.size(); ++i) {
        ...
        // OJO PROFE: Como mi BD devuelve el Top 10, y el índice 11 son "mis stats" (si no estaba en el top),
        // añado un hueco en Y para separar los 10 primeros de mi puntuación de mierda.
        const size_t TOP_LIMIT = 10;
        if (i >= TOP_LIMIT) y += HIGHLIGHT_OFFSET;

        // Ilumino la fila donde esté mi nombre
        sf::Color rowColor = (rankingData[i].name == myName) ? sf::Color::Yellow : sf::Color::White;

        sf::Text rowPos(font); rowPos.setString(std::to_string(rankingData[i].pos) + "."); rowPos.setCharacterSize(FONT_SIZE); rowPos.setPosition({ X_POS, y }); rowPos.setFillColor(rowColor); window.draw(rowPos);
        sf::Text rowName(font); rowName.setString(rankingData[i].name); rowName.setCharacterSize(FONT_SIZE); rowName.setPosition({ X_NAME, y }); rowName.setFillColor(rowColor); window.draw(rowName);
        sf::Text rowPts(font); rowPts.setString(std::to_string(rankingData[i].pts)); rowPts.setCharacterSize(FONT_SIZE); rowPts.setPosition({ X_PTS, y }); rowPts.setFillColor(rowColor); window.draw(rowPts);
        sf::Text rowW(font); rowW.setString(std::to_string(rankingData[i].w)); rowW.setCharacterSize(FONT_SIZE); rowW.setPosition({ X_W, y }); rowW.setFillColor(rowColor); window.draw(rowW);
        sf::Text rowL(font); rowL.setString(std::to_string(rankingData[i].l)); rowL.setCharacterSize(FONT_SIZE); rowL.setPosition({ X_L, y }); rowL.setFillColor(rowColor); window.draw(rowL);
    }
