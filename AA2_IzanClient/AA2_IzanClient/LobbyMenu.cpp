...
LobbyMenu::LobbyMenu()
    : titleText(font), createText(font), joinText(font), inputLabel(font), displayRoomCode(font), boxActive(false), inRoom(false), myPlayerID(0)
{
            ...
            if (resolvedIPs.has_value() && !resolvedIPs->empty()) {
                if (socket.connect((*resolvedIPs)[0], 50000) == sf::Socket::Status::Done) {
                    ...
                    if (socket.receive(respuesta) == sf::Socket::Status::Done) {
                        ...
                        respuesta >> estado;
                        if (estado == 1) {
                            if (isCreate) respuesta >> roomCode >> myPlayerID;
                            else respuesta >> myPlayerID;
